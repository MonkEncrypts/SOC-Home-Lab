...
To verify that Splunk is ingesting a variety of Sysmon telemetry rather than a single event type, the following SPL query was executed:
...
![Sysmon Event ID Distribution](screenshots/splunk-eventid-distribution.jpeg)
*Figure 3 — Distribution of Sysmon Event IDs indexed by Splunk, confirming successful ingestion of multiple event types generated by the monitored Windows endpoint.*

The resulting distribution confirms that the telemetry pipeline is successfully collecting and indexing a diverse range of Sysmon events. This provides sufficient endpoint visibility for developing and validating detection rules based on multiple MITRE ATT&CK techniques.
